...
async function generateActionPreview(
  ...
  switch (toolName) {
    ...
    case 'update_lead': {
      ...
      if (args.newPhone) changes.push(`Phone: ${lead.phone_number || 'none'} → ${args.newPhone}`)
      if (args.newEmail) changes.push(`Email: ${lead.email || 'none'} → ${args.newEmail}`)
      if (args.newAddress) changes.push(`Address: ${lead.address || 'none'} → ${args.newAddress}`)
    ...
    case 'edit_quote': {
      ...
      if (args.newDiscountPercentage !== undefined) {
        changes.push(`Discount: ${quote.discount_percentage || 0}% → ${args.newDiscountPercentage}%`)
      }
      if (args.newAddress) {
        changes.push(`Address: ${quote.address || 'none'} → ${args.newAddress}`)
      }
      if (args.newNotes) {
